...
class Element(object):
    ...
    @property
    def _max_abundance_weight(self) -> float:
        isotopic_weight = self.isotopic_weight
        ratios = self._periodic_table["isotopic_ratio"]
        maxindex = ratios.index(max(ratios))
        return isotopic_weight[maxindex]
        
    @property
    def iso_molecular_weight(self) -> float:
        return self._max_abundance_weight * float(self.count)
